...
const DetailData = ({navigation, route}) => {
    ...
    useEffect(() => {
        axios.get('https://mighty-spire-11680.herokuapp.com/produk/' + route.params.id)
        .then(res => {
            setNama_paket(res.data.nama_paket);
            setTipe_paket(res.data.tipe_paket);
            setHarga(res.data.harga);
            setJml_kouta(res.data.jml_kouta);
            setMasa_aktif(res.data.masa_aktif);
        })
        .catch(err => {
            console.log(err);
        })
    ...
    const Update = () => {
        if(nama_paket == '' || tipe_paket == '' || harga == '' || jml_kouta == '' || masa_aktif == ''){
            ...
            axios.put('https://mighty-spire-11680.herokuapp.com/produk/'+route.params.id, {
            ...
            .then(res => {
                ...
                [
                    {text: 'OK', onPress: () => navigation.replace('ViewData', {
                        username: username
                    })}
                ],
                {cancelable: false}
    ...
    const Delete = () => {
        ...
        [
            {text: 'OK', onPress: () => {
                axios.delete('https://mighty-spire-11680.herokuapp.com/produk/'+route.params.id)
                .then(res => {
                    console.log(res);
                    console.log(res.data);
                    Alert.alert('Sukses', 
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                <TouchableOpacity onPress={() => navigation.replace('ViewData', {
                    username: username
                })}>
